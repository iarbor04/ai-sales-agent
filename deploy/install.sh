...
# Ключ подписки, если он передан установщику. Не передан — не беда: владелец
# введёт его в панели, на странице «Подписка».
#   LICENSE_KEY=ascn-xxxx bash deploy/install.sh
if [ -n "${LICENSE_KEY:-}" ]; then
  set_env LICENSE_KEY "$LICENSE_KEY"
  set_env LICENSE_REQUIRED 1
fi
...
echo "========================================"
echo
if [ "$(get_env LICENSE_REQUIRED)" = "1" ]; then
  echo "Подписка ASCN включена. Если ключ не задан, владелец вводит его в панели,"
  echo "страница «Подписка» — до этого агент не отвечает клиентам."
  echo "Бота владелец добавляет сам, раздел «Боты»; модель включена в подписку."
else
